...
# 计算某一属性的信息增益
def infoGainCalc(xfeature, y, entD, is_continuous = True):
...
def chooseBestFeatureInfoGain(X, y):
    ...
    for featName in features:
        # 判断属性取值是否为连续值
        # is_continuous = is_continuous(X[featName])
        info_gain = infoGainCalc(X[featName], y, entD)
        # 更新最优属性
...
if __name__ == '__main__':
    ...
    # 读入数据
    dataPath = r'D:\\vscode\\Markdown\\ML\\haberman.txt'
